...
namespace Library
{
	...
	bool Device::DrawStart()
	{
		...
		pDevice->SetRenderState(D3DRS_ZENABLE, true); // Zバッファー
		//pDevice->SetRenderState(D3DRS_CULLMODE, true);　// カリング
		//pDevice->SetRenderState(D3DRS_SRCBLEND, D3DBLEND_SRCALPHA); // 上から新しく描画する色情報(SRC)
		//pDevice->SetRenderState(D3DRS_DESTBLEND, D3DBLEND_INVSRCALPHA);// 既に表示されている色情報(DEST)
